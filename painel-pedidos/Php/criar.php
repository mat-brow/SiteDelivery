...
$erro = '';
$d = ['cliente' => '', 'itens' => '', 'valor_total' => '', 'status' => 'pendente'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($d['cliente'] === '' || $d['itens'] === '') {
        ...
        $stmt = $conn->prepare('INSERT INTO pedidos (cliente, itens, valor_total, status) VALUES (?, ?, ?, ?)');
        $stmt->bind_param('ssds', $d['cliente'], $d['itens'], $valor, $d['status']);
        $stmt->execute();
...
<head>
    ...
    <title>Novo pedido | Painel de Pedidos</title>
...
<main class="container py-4" style="max-width: 720px;">
    ...
    <form method="post" class="card painel p-4 needs-validation" novalidate>
        ...
        <div class="d-flex gap-2">
            <button class="btn btn-brand" type="submit">Criar pedido</button>
